...
This page lists every function available in SuperPlane expressions. For an introduction, see the [expression guide](/expressions/guide).
...
Array functions that accept a predicate use `#` for the current element. See [Closures](/expressions/guide#closures-).
...
| ------ | ------- | ------- |
| `.Year()` | Year, such as 2024 | `now().Year()` |
