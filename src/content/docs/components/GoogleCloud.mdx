...
<CardGrid>
  ...
  <LinkCard title="Compute • Create Firewall Rule" href="#compute-•-create-firewall-rule" description="Create a VPC firewall rule that allows or denies traffic to or from your VM instances" />
  <LinkCard title="Compute • Create Load Balancer" href="#compute-•-create-load-balancer" description="Create a regional external passthrough Network Load Balancer that forwards TCP/UDP traffic to a group of VM instances" />
  ...
  <LinkCard title="Compute • Delete Firewall Rule" href="#compute-•-delete-firewall-rule" description="Permanently delete a VPC firewall rule" />
  <LinkCard title="Compute • Delete Load Balancer" href="#compute-•-delete-load-balancer" description="Delete a regional external passthrough Network Load Balancer and its backend service and health check" />
  ...
  <LinkCard title="Compute • Update Firewall Rule" href="#compute-•-update-firewall-rule" description="Update a VPC firewall rule: its protocols and ports, ranges, priority, targets and source filters, description, or enabled state" />
  ...
  <LinkCard title="Monitoring • Create Alerting Policy" href="#monitoring-•-create-alerting-policy" description="Create a Cloud Monitoring alerting policy from an instance-metric threshold or a PromQL query (Managed Prometheus)" />
  <LinkCard title="Monitoring • Create Snooze" href="#monitoring-•-create-snooze" description="Create a Cloud Monitoring snooze that suppresses alert notifications for selected policies over a time window" />
  ...
  <LinkCard title="Monitoring • Expire Snooze" href="#monitoring-•-expire-snooze" description="End an active Cloud Monitoring snooze now so the snoozed policies can notify again" />
  ...
  <LinkCard title="Monitoring • Get Snooze" href="#monitoring-•-get-snooze" description="Read a Cloud Monitoring snooze, including the policies it covers and its time window" />
  ...
  <LinkCard title="Cloud Storage • Create Bucket" href="#cloud-storage-•-create-bucket" description="Create a Cloud Storage bucket" />
  <LinkCard title="Cloud Storage • Delete Bucket" href="#cloud-storage-•-delete-bucket" description="Delete a Cloud Storage bucket" />
  <LinkCard title="Cloud Storage • Get Bucket" href="#cloud-storage-•-get-bucket" description="Fetch a Cloud Storage bucket's configuration and metadata" />
...
<a id="compute-•-create-firewall-rule"></a>

## Compute • Create Firewall Rule

**Component key:** `gcp.compute.createFirewallRule`

The Create Firewall Rule component creates a VPC firewall rule that controls traffic to and from the VM instances in a network.
...
```json
{
  ...
  "type": "gcp.compute.firewallRule.created"
}
```

<a id="compute-•-create-load-balancer"></a>

## Compute • Create Load Balancer

**Component key:** `gcp.compute.createLoadBalancer`
...
```json
{
  "data": {
    ...
    "forwardingRule": "https://www.googleapis.com/compute/v1/projects/my-project/regions/us-central1/forwardingRules/web-lb-fr",
    "healthCheck": "web-lb-hc",
    "instanceGroup": "web-servers",
    "ipAddress": "34.1.2.3",
    "name": "web-lb",
    "ports": [
      "80",
    ...
    "region": "us-central1",
    "resources": [
      "https://www.googleapis.com/compute/v1/projects/my-project/regions/us-central1/healthChecks/web-lb-hc",
      "https://www.googleapis.com/compute/v1/projects/my-project/regions/us-central1/backendServices/web-lb-backend",
      "https://www.googleapis.com/compute/v1/projects/my-project/regions/us-central1/forwardingRules/web-lb-fr"
    ]
  },
  "timestamp": "2025-02-14T12:00:00Z",
  "type": "gcp.compute.loadBalancer.created"
}
```
...
<a id="compute-•-delete-firewall-rule"></a>

## Compute • Delete Firewall Rule

**Component key:** `gcp.compute.deleteFirewallRule`

The Delete Firewall Rule component permanently deletes a VPC firewall rule.
...
```json
{
  ...
  "timestamp": "2026-06-23T12:00:00Z",
  "type": "gcp.compute.firewallRule.deleted"
}
```

<a id="compute-•-delete-load-balancer"></a>

## Compute • Delete Load Balancer

**Component key:** `gcp.compute.deleteLoadBalancer`
...
```json
{
  ...
  "timestamp": "2025-02-14T12:00:00Z",
  "type": "gcp.compute.loadBalancer.deleted"
}
```
...
<a id="compute-•-update-firewall-rule"></a>

## Compute • Update Firewall Rule

**Component key:** `gcp.compute.updateFirewallRule`

The Update Firewall Rule component changes an existing VPC firewall rule. **Toggle on only the fields you want to change; everything left off is untouched.** Enabled state, protocols and ports, targets, source filter and logs are dropdowns you toggle on and then pick a value.
...
```json
{
  ...
  "timestamp": "2026-06-23T12:00:00Z",
  "type": "gcp.compute.firewallRule.updated"
}
```
...
<a id="monitoring-•-create-snooze"></a>

## Monitoring • Create Snooze

**Component key:** `gcp.monitoring.createSnooze`
...
{
  "data": {
    "displayName": "Deploy maintenance window",
    "endTime": "2025-02-14T13:00:00Z",
    "id": "1234567890123456789",
    "name": "projects/my-project/snoozes/1234567890123456789",
    "policies": [
      "projects/my-project/alertPolicies/1111111111111111111"
    ],
    "policiesCount": 1,
    "startTime": "2025-02-14T12:00:00Z"
  },
  "timestamp": "2025-02-14T12:00:00Z",
  "type": "gcp.monitoring.snooze.created"
}
```
...
<a id="monitoring-•-expire-snooze"></a>

## Monitoring • Expire Snooze

**Component key:** `gcp.monitoring.expireSnooze`
...
```json
{
  ...
  "type": "gcp.monitoring.snooze.expired"
}
```
...
<a id="monitoring-•-get-snooze"></a>

## Monitoring • Get Snooze

**Component key:** `gcp.monitoring.getSnooze`
...
```json
{
  ...
  "timestamp": "2025-02-14T12:30:00Z",
  "type": "gcp.monitoring.snooze.fetched"
}
```
...
<a id="cloud-storage-•-create-bucket"></a>

## Cloud Storage • Create Bucket

**Component key:** `gcp.storage.createBucket`

The Create Bucket component creates a new Cloud Storage bucket.
...
<a id="cloud-storage-•-delete-bucket"></a>

## Cloud Storage • Delete Bucket

**Component key:** `gcp.storage.deleteBucket`

The Delete Bucket component permanently deletes a Cloud Storage bucket.
...
<a id="cloud-storage-•-get-bucket"></a>

## Cloud Storage • Get Bucket

**Component key:** `gcp.storage.getBucket`

The Get Bucket component retrieves a Cloud Storage bucket's configuration and metadata.
