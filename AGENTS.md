...
- `src/content/docs/` - All documentation content (`.md` and `.mdx` files)
  ...
  - Organized by topic (for example, `get-started/` and `fundamentals/`)
...
- SHOULD: Link a term's first mention to a relevant documentation page when one exists.
...
- MUST: Internal links use site-root paths (`/get-started/overview`, `/components`).
...
- Prefer Starlight **autogenerated sections** (`autogenerate`) for directories that will grow (for example,
  `components/`).
...
| Reference | `src/content/docs/components/GitHub.mdx` |
